...
app.patch('/api/listings/:id', async (req, res) => {
  try {
    ...
    const [row] = await sql`UPDATE listings SET status=${status}, updated_at=now() WHERE id=${req.params.id} RETURNING *`
    res.json(row)
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
app.patch('/api/bookings/:id', async (req, res) => {
  try {
    const { status } = req.body
    const [row] = await sql`UPDATE bookings SET status=${status}, updated_at=now() WHERE id=${req.params.id} RETURNING *`
    res.json(row)
  } catch (e) { res.status(500).json({ error: e.message }) }
})
